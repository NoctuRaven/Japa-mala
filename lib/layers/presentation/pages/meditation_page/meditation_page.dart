...
class MeditationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void onMeditationListTap(MeditationEntity meditation) {
      meditationIndex = context
          .read<DatabaseMeditationCubit>()
          .onGetMeditationIndex(meditation);
      context.read<DatabaseMeditationCubit>().onGetMeditationList();
      Navigator.pop(context);
    }

    void onDeleteMeditation(MeditationEntity meditation) {
      if (meditationList.isNotEmpty) {
        int index =
            context.read<DatabaseMeditationCubit>().state.indexOf(meditation);
        index > 0 ? meditationIndex = index - 1 : meditationIndex = index + 1;
      }
      context.read<DatabaseMeditationCubit>().onDeleteMeditation(meditation);
    }
    ...
    return Visibility(
      child: BlocBuilder<DatabaseMeditationCubit, List<MeditationEntity>>(
        builder: (context, state) {
          var timerCubit =
              state.isNotEmpty ? TimerCubit(state[meditationIndex]) : null;
          return Scaffold(
            drawer:
                 Drawer(
